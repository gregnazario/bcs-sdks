...
public class BcsError extends RuntimeException {
    ...
    public static BcsError nonCanonicalUleb128() {
        return new BcsError(Type.NON_CANONICAL_ULEB128, "Non-canonical ULEB128 encoding (trailing zero bytes)");
    }
    ...
    public static BcsError exceededContainerDepth(String container) {
        String msg = (container != null && !container.isEmpty())
    ...
    public static BcsError remainingInput(int remaining) {
        return new BcsError(
                Type.REMAINING_INPUT, String.format("Remaining input after deserialization: %d bytes", remaining));
    }

    public static BcsError nonCanonicalMap(String reason) {
        return new BcsError(
                Type.NON_CANONICAL_MAP,
                String.format("Non-canonical map: %s", reason != null ? reason : "keys not sorted or contain duplicates"));
    }

    public static BcsError unknownVariant(int index, int maxIndex) {
        return new BcsError(
                Type.UNKNOWN_VARIANT, String.format("Unknown enum variant index: %d (max: %d)", index, maxIndex));
    }
    ...
    public static BcsError valueOutOfRange(String typeName, Object value) {
        return new BcsError(Type.VALUE_OUT_OF_RANGE, String.format("%s value out of range: %s", typeName, value));
    }
}
